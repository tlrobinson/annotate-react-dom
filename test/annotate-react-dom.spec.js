...
import {
  annotateReactNode,
  annotateReactNodes
} from "../src/annotate-react-dom";

const Hello = ({ children }) => <div>Hello {children}</div>;
...
describe("annotate-react-dom", () => {
  ...
  describe("annotateReactNodes", () => {
    it("should annotate a React component's DOM node with custom attribute", () => {
      ...
      expect(container.querySelector("[react=Hello]").textContent).toBe(
        "Hello World"
      );
      expect(
        container.querySelector("[react=Hello] [react=World]").textContent
      ).toBe("World");
    });
  });
});
